...
$("#btn_Add").click(function() {
	...
	$("#productTypeId").textbox('setValue', "");
...
$("#btn_Edit").click(function() {
	...
	$("#productCarriage").textbox('setValue', row.productCarriage);
	$("#productAddressRigger").textbox('setValue', row.productAddressRigger);
	$("#productSendCredit").textbox('setValue', row.productSendCredit);
	$("#productShopName").textbox('setValue', row.productShopName);
	$("#productShopUrl").textbox('setValue', row.productShopUrl);
	$("#inventory").textbox('setValue', row.inventory);
	$("#productTypeId").textbox('setValue', row.productTypeId);
	$("#imgName").textbox('setValue', row.productImg);
	$("#productImg").val(row.productImg);
	imgName = row.productImg;
});
...
function saveData() {
	...
	var productCarriage = $("#productCarriage").textbox("getValue");
	var productAddressRigger = $("#productAddressRigger").textbox("getValue");
	var productSendCredit = $("#productSendCredit").textbox("getValue");
	var productShopName = $("#productShopName").textbox("getValue");
	var productShopUrl = $("#productShopUrl").textbox("getValue");
	...
	$.post(url, {
		...
		"productCarriage" : productCarriage,
		"productAddressRigger" : productAddressRigger,
		"productSendCredit" : productSendCredit,
		"productShopName" : productShopName,
		"productShopUrl" : productShopUrl,
		"inventory" : inventory,
		"productImg" : productImg
	}, function(data) {
		if (data.success) {
...
var uploader = Qiniu.uploader({
	...
	domain : 'http://pt655rcn6.bkt.clouddn.com/', // bucket域名，下载资源时用到，必需
	...
	uptoken_func : function(file) { // 将key传给服务端获取的七牛token，上传后覆盖同名文件，即为更新操作
		var val = $("#productImg").val();// val用于保存上传文件的url
	...
	init : {
		...
		'FileUploaded' : function(up, file, info) { // 每个文件上传成功后，处理相关的事情
			...
			$("#productImg").val(domain + data.key);
			saveData();
		},
